...
import { createRestaurant, updateRestaurant, getAllRestaurants, deleteRestaurant } from './Actions';
...
describe('Restaurant Server Actions', () => {
    ...
    it('should create, modify, retrieve, and delete restaurants', async () => {
      ...
      const restaurantsData = [
        {
          ...
          workingHours: ['Mon-Fri: 11:00 AM - 10:00 PM', 'Sat-Sun: 12:00 PM - 11:00 PM'],
          location: {
            type: 'Point' as const,
        ...
        },
        {
          ...
          workingHours: ['Mon-Thu: 11:00 AM - 9:00 PM', 'Fri-Sun: 11:00 AM - 11:00 PM'],
          location: {
            type: 'Point' as const,
      ...
      for (const restaurantData of restaurantsData) {
        ...
        console.log(`Created restaurant: ${restaurantData.name} with ID: ${result.data.id}`);
      }
      ...
      const updateResult1 = await updateRestaurant(
        createdRestaurantIds[0],
        {
      ...
      const updateResult2 = await updateRestaurant(
        createdRestaurantIds[2],
        {
          name: 'Burger Joint Premium',
          imageUrl: 'https://example.com/burger-joint-premium.jpg',
          location: {
            type: 'Point' as const,
            coordinates: [-87.630000, 41.878200] // Slightly different location
          }
        }
      );
      ...
      const updateResult3 = await updateRestaurant(
        createdRestaurantIds[4],
        {
      ...
      console.log(`Retrieved ${getAllResult.data.length} restaurants (sorted by name):`);
      ...
      const pizzaParadise = getAllResult.data.find((r: any) => r.name === 'Pizza Paradise');
      ...
      const burgerJoint = getAllResult.data.find((r: any) => r.name === 'Burger Joint Premium');
      expect(burgerJoint).toBeDefined();
      expect(burgerJoint.imageUrl).toBe('https://example.com/burger-joint-premium.jpg');
      expect(burgerJoint.location.coordinates[0]).toBeCloseTo(-87.630000);
  
      const pastaPalace = getAllResult.data.find((r: any) => r.name === 'Pasta Palace');
    ...
    it('should handle errors correctly', async () => {
      ...
      const invalidUpdateResult = await updateRestaurant(
        'invalid-id',
    ...
    it('should handle geospatial queries correctly', async () => {
      ...
      const central = allRestaurants.data.find((r: any) => r.name === 'Central Restaurant');
    ...
    it('should handle updating multiple restaurants independently', async () => {
      ...
      const restaurant2 = await createRestaurant({
        ...
        location: {
          type: 'Point',
          coordinates: [-74.006000, 40.714000]
        }
      });
      ...
      // Verify both updates
      const allRestaurants = await getAllRestaurants();
      const updatedA = allRestaurants.data.find((r: any) => r._id === restaurant1.data.id);
      const updatedB = allRestaurants.data.find((r: any) => r._id === restaurant2.data.id);
    ...
    it('should handle very long working hours arrays', async () => {
      ...
      const restaurants = await getAllRestaurants();
      const created = restaurants.data.find((r: any) => r.name === 'Detailed Hours Restaurant');
    ...
    it('should delete the correct restaurant and not affect others', async () => {
      ...
      const r3 = await createRestaurant({
        ...
        location: {
          type: 'Point',
          coordinates: [-75.000000, 41.000000]
        }
      });
